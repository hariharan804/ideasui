export const lightColorTokens = {
  primary: {
    50: 'oklch(0.970 0.020 277.1)',
    100: 'oklch(0.930 0.037 277.1)',
    200: 'oklch(0.870 0.071 277.1)',
    300: 'oklch(0.790 0.112 277.1)',
    400: 'oklch(0.700 0.159 277.1)',
    500: 'oklch(0.590 0.204 277.1)',
    600: 'oklch(0.500 0.194 277.1)',
    700: 'oklch(0.420 0.173 277.1)',
    800: 'oklch(0.320 0.147 277.1)',
    900: 'oklch(0.220 0.112 277.1)',
    950: 'oklch(0.140 0.082 277.1)',
  },
  secondary: {
    50: 'oklch(0.970 0.015 237.3)',
    100: 'oklch(0.930 0.027 237.3)',
    200: 'oklch(0.870 0.052 237.3)',
    300: 'oklch(0.790 0.081 237.3)',
    400: 'oklch(0.700 0.115 237.3)',
    500: 'oklch(0.590 0.148 237.3)',
    600: 'oklch(0.500 0.140 237.3)',
    700: 'oklch(0.420 0.126 237.3)',
    800: 'oklch(0.320 0.106 237.3)',
    900: 'oklch(0.220 0.081 237.3)',
    950: 'oklch(0.140 0.059 237.3)',
  },
  tertiary: {
    50: 'oklch(0.970 0.022 292.7)',
    100: 'oklch(0.930 0.039 292.7)',
    200: 'oklch(0.870 0.077 292.7)',
    300: 'oklch(0.790 0.120 292.7)',
    400: 'oklch(0.700 0.171 292.7)',
    500: 'oklch(0.590 0.219 292.7)',
    600: 'oklch(0.500 0.208 292.7)',
    700: 'oklch(0.420 0.186 292.7)',
    800: 'oklch(0.320 0.158 292.7)',
    900: 'oklch(0.220 0.120 292.7)',
    950: 'oklch(0.140 0.088 292.7)',
  },
  warning: {
    50: 'oklch(0.970 0.016 70.1)',
    100: 'oklch(0.930 0.030 70.1)',
    200: 'oklch(0.870 0.058 70.1)',
    300: 'oklch(0.790 0.091 70.1)',
    400: 'oklch(0.700 0.128 70.1)',
    500: 'oklch(0.590 0.165 70.1)',
    600: 'oklch(0.500 0.156 70.1)',
    700: 'oklch(0.420 0.140 70.1)',
    800: 'oklch(0.320 0.119 70.1)',
    900: 'oklch(0.220 0.091 70.1)',
    950: 'oklch(0.140 0.066 70.1)',
  },
  success: {
    50: 'oklch(0.970 0.019 149.6)',
    100: 'oklch(0.930 0.035 149.6)',
    200: 'oklch(0.870 0.067 149.6)',
    300: 'oklch(0.790 0.106 149.6)',
    400: 'oklch(0.700 0.150 149.6)',
    500: 'oklch(0.590 0.192 149.6)',
    600: 'oklch(0.500 0.182 149.6)',
    700: 'oklch(0.420 0.163 149.6)',
    800: 'oklch(0.320 0.138 149.6)',
    900: 'oklch(0.220 0.106 149.6)',
    950: 'oklch(0.140 0.077 149.6)',
  },
  danger: {
    50: 'oklch(0.970 0.021 25.3)',
    100: 'oklch(0.930 0.037 25.3)',
    200: 'oklch(0.870 0.073 25.3)',
    300: 'oklch(0.790 0.114 25.3)',
    400: 'oklch(0.700 0.162 25.3)',
    500: 'oklch(0.590 0.208 25.3)',
    600: 'oklch(0.500 0.197 25.3)',
    700: 'oklch(0.420 0.177 25.3)',
    800: 'oklch(0.320 0.150 25.3)',
    900: 'oklch(0.220 0.114 25.3)',
    950: 'oklch(0.140 0.083 25.3)',
  },
  info: {
    50: 'oklch(0.970 0.014 242.0)',
    100: 'oklch(0.930 0.025 242.0)',
    200: 'oklch(0.870 0.049 242.0)',
    300: 'oklch(0.790 0.076 242.0)',
    400: 'oklch(0.700 0.108 242.0)',
    500: 'oklch(0.590 0.139 242.0)',
    600: 'oklch(0.500 0.132 242.0)',
    700: 'oklch(0.420 0.118 242.0)',
    800: 'oklch(0.320 0.100 242.0)',
    900: 'oklch(0.220 0.076 242.0)',
    950: 'oklch(0.140 0.056 242.0)',
  },
  neutral: {
    50: 'oklch(0.970 0.008 257.4)',
    100: 'oklch(0.930 0.014 257.4)',
    200: 'oklch(0.870 0.028 257.4)',
    300: 'oklch(0.790 0.044 257.4)',
    400: 'oklch(0.700 0.062 257.4)',
    500: 'oklch(0.590 0.080 257.4)',
    600: 'oklch(0.500 0.076 257.4)',
    700: 'oklch(0.420 0.068 257.4)',
    800: 'oklch(0.320 0.058 257.4)',
    900: 'oklch(0.220 0.044 257.4)',
    950: 'oklch(0.140 0.032 257.4)',
  },
  gray: {
    50: 'oklch(0.970 0.000 0.0)',
    100: 'oklch(0.930 0.000 0.0)',
    200: 'oklch(0.870 0.000 0.0)',
    300: 'oklch(0.790 0.000 0.0)',
    400: 'oklch(0.700 0.000 0.0)',
    500: 'oklch(0.590 0.000 0.0)',
    600: 'oklch(0.500 0.000 0.0)',
    700: 'oklch(0.420 0.000 0.0)',
    800: 'oklch(0.320 0.000 0.0)',
    900: 'oklch(0.220 0.000 0.0)',
    950: 'oklch(0.140 0.000 0.0)',
  },
} as const;

export const darkColorTokens = {
  primary: {
    50: 'oklch(0.140 0.082 277.1)',
    100: 'oklch(0.200 0.102 277.1)',
    200: 'oklch(0.280 0.126 277.1)',
    300: 'oklch(0.380 0.153 277.1)',
    400: 'oklch(0.480 0.180 277.1)',
    500: 'oklch(0.590 0.204 277.1)',
    600: 'oklch(0.700 0.173 277.1)',
    700: 'oklch(0.790 0.139 277.1)',
    800: 'oklch(0.870 0.098 277.1)',
    900: 'oklch(0.930 0.057 277.1)',
    950: 'oklch(0.970 0.031 277.1)',
  },
  secondary: {
    50: 'oklch(0.140 0.059 237.3)',
    100: 'oklch(0.200 0.074 237.3)',
    200: 'oklch(0.280 0.092 237.3)',
    300: 'oklch(0.380 0.111 237.3)',
    400: 'oklch(0.480 0.130 237.3)',
    500: 'oklch(0.590 0.148 237.3)',
    600: 'oklch(0.700 0.126 237.3)',
    700: 'oklch(0.790 0.101 237.3)',
    800: 'oklch(0.870 0.071 237.3)',
    900: 'oklch(0.930 0.041 237.3)',
    950: 'oklch(0.970 0.022 237.3)',
  },
  tertiary: {
    50: 'oklch(0.140 0.088 292.7)',
    100: 'oklch(0.200 0.109 292.7)',
    200: 'oklch(0.280 0.136 292.7)',
    300: 'oklch(0.380 0.164 292.7)',
    400: 'oklch(0.480 0.193 292.7)',
    500: 'oklch(0.590 0.219 292.7)',
    600: 'oklch(0.700 0.186 292.7)',
    700: 'oklch(0.790 0.149 292.7)',
    800: 'oklch(0.870 0.105 292.7)',
    900: 'oklch(0.930 0.061 292.7)',
    950: 'oklch(0.970 0.033 292.7)',
  },
  warning: {
    50: 'oklch(0.140 0.066 70.1)',
    100: 'oklch(0.200 0.082 70.1)',
    200: 'oklch(0.280 0.102 70.1)',
    300: 'oklch(0.380 0.124 70.1)',
    400: 'oklch(0.480 0.145 70.1)',
    500: 'oklch(0.590 0.165 70.1)',
    600: 'oklch(0.700 0.140 70.1)',
    700: 'oklch(0.790 0.112 70.1)',
    800: 'oklch(0.870 0.079 70.1)',
    900: 'oklch(0.930 0.046 70.1)',
    950: 'oklch(0.970 0.025 70.1)',
  },
  success: {
    50: 'oklch(0.140 0.077 149.6)',
    100: 'oklch(0.200 0.096 149.6)',
    200: 'oklch(0.280 0.119 149.6)',
    300: 'oklch(0.380 0.144 149.6)',
    400: 'oklch(0.480 0.169 149.6)',
    500: 'oklch(0.590 0.192 149.6)',
    600: 'oklch(0.700 0.163 149.6)',
    700: 'oklch(0.790 0.131 149.6)',
    800: 'oklch(0.870 0.092 149.6)',
    900: 'oklch(0.930 0.054 149.6)',
    950: 'oklch(0.970 0.029 149.6)',
  },
  danger: {
    50: 'oklch(0.140 0.083 25.3)',
    100: 'oklch(0.200 0.104 25.3)',
    200: 'oklch(0.280 0.129 25.3)',
    300: 'oklch(0.380 0.156 25.3)',
    400: 'oklch(0.480 0.183 25.3)',
    500: 'oklch(0.590 0.208 25.3)',
    600: 'oklch(0.700 0.177 25.3)',
    700: 'oklch(0.790 0.141 25.3)',
    800: 'oklch(0.870 0.100 25.3)',
    900: 'oklch(0.930 0.058 25.3)',
    950: 'oklch(0.970 0.031 25.3)',
  },
  info: {
    50: 'oklch(0.140 0.056 242.0)',
    100: 'oklch(0.200 0.069 242.0)',
    200: 'oklch(0.280 0.086 242.0)',
    300: 'oklch(0.380 0.104 242.0)',
    400: 'oklch(0.480 0.122 242.0)',
    500: 'oklch(0.590 0.139 242.0)',
    600: 'oklch(0.700 0.118 242.0)',
    700: 'oklch(0.790 0.094 242.0)',
    800: 'oklch(0.870 0.067 242.0)',
    900: 'oklch(0.930 0.039 242.0)',
    950: 'oklch(0.970 0.021 242.0)',
  },
  neutral: {
    50: 'oklch(0.140 0.032 257.4)',
    100: 'oklch(0.200 0.040 257.4)',
    200: 'oklch(0.280 0.050 257.4)',
    300: 'oklch(0.380 0.060 257.4)',
    400: 'oklch(0.480 0.070 257.4)',
    500: 'oklch(0.590 0.080 257.4)',
    600: 'oklch(0.700 0.068 257.4)',
    700: 'oklch(0.790 0.054 257.4)',
    800: 'oklch(0.870 0.038 257.4)',
    900: 'oklch(0.930 0.022 257.4)',
    950: 'oklch(0.970 0.012 257.4)',
  },
  gray: {
    50: 'oklch(0.140 0.000 0.0)',
    100: 'oklch(0.200 0.000 0.0)',
    200: 'oklch(0.280 0.000 0.0)',
    300: 'oklch(0.380 0.000 0.0)',
    400: 'oklch(0.480 0.000 0.0)',
    500: 'oklch(0.590 0.000 0.0)',
    600: 'oklch(0.700 0.000 0.0)',
    700: 'oklch(0.790 0.000 0.0)',
    800: 'oklch(0.870 0.000 0.0)',
    900: 'oklch(0.930 0.000 0.0)',
    950: 'oklch(0.970 0.000 0.0)',
  },
} as const;
