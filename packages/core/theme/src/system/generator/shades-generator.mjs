...
export function generateShades(baseColor, colorName, format = "oklch") {
  ...
  const lightShades = {
    50: formatColor([0.97, chromaBase * 0.10, finalHue], format),   // Very light
    ...
    400: formatColor([0.70, chromaBase * 0.78, finalHue], format),
    500: formatColor([0.59, chromaBase, finalHue], format),          // Base - peak chroma
    600: formatColor([0.50, chromaBase * 0.95, finalHue], format),
    ...
    950: formatColor([0.14, chromaBase * 0.40, finalHue], format),   // Very dark
  };

  // Dark mode: Inverted progression (50 is dark, 950 is light)
  // For dark backgrounds, we flip the scale
  const darkShades = {
    50: formatColor([0.14, chromaBase * 0.40, finalHue], format),   // Very dark
    100: formatColor([0.20, chromaBase * 0.50, finalHue], format),
    ...
    500: formatColor([0.59, chromaBase, finalHue], format),          // Base - peak chroma
    600: formatColor([0.70, chromaBase * 0.85, finalHue], format),
