...
export function CopyDropdown({ rawMarkdown, pageTitle }: CopyDropdownProps) {
...
I have the above context. Please help me with my task or question regarding this component:`;
    ...
    if (isLocal) {
      ...
      const rawDocUrl =
        typeof window !== 'undefined'
          ? `${window.location.origin}${pathname.replace(/^\/react\/docs/, '/api/raw-doc')}.mdx`
          : '';
      ...
      const queryParam = aiName === 'ChatGPT' ? 'prompt' : 'q';

      const fullUrl = `${targetUrl}?${queryParam}=${encodedPrompt}`;
                      ...
                      <div className="relative">
                        <button
                          ...
                          onClick={() => {
                            ...
                            copyToClipboard(configStr, 'VS Code configuration JSON');
                          }}
                        >
                          <Copy className="size-3" />
